...
public class ProductJdbcRepository implements ProductRepository {
    ...
    public Product update(Product product) {
        int update = jdbcTemplate.update(
                "UPDATE products SET product_name = :productName, category = :category, price = :price, description = :description, created_at = :createdAt, updated_at = :updatedAt WHERE product_id = UUID_TO_BIN(:productId)",
                toParamMap(product)
        );
        if (update != 1) {
